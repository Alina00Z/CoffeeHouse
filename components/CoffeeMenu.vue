...
<div class="container">
  <div class="window_coffee">
    ...
    <div v-if="tabsState[0].active" class="window-coffee-grid">
      <coffee-menu-card @click-card="$emit('click-card', $event)" v-for="(card,i) in cards_coffee" :key="i" :type="'coffee'" :name="card.name" :info="card.info" :price="card.price"/>
    </div>
    <div v-if="tabsState[1].active" class="window-coffee-grid">
      <coffee-menu-card @click-card="$emit('click-card', $event)" v-for="(card,i) in cards_tea" :key="i" :type="'tea'" :name="card.name" :info="card.info" :price="card.price"/>
    </div>
    <div v-if="tabsState[2].active" class="window-coffee-grid">
      <coffee-menu-card @click-card="$emit('click-card', $event)" v-for="(card,i) in cards_dessert" :key="i" :type="'dessert'" :name="card.name" :info="card.info" :price="card.price"/>
    </div>
